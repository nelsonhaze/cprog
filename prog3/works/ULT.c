...
Tid ULT_DestroyThread(Tid wantTid)
{
  ...
  ThrdCtlBlk * tcb_to_zombify;
  ThrdCtlBlk * tcb_to_run;
 /*  kill zombies!!!! wooo hooo
        ___________                .....            
     ~Y/ =====-__|~==             C C  /            
       / |L)~~~                  /<   /             
  ...
  */ 
  ...
  // if we are looking on the ready list wantTid cannot be self!
  else {
    tcb_to_zombify = findTcb(wantTid);
    if(!tcb_to_zombify) {
      return ULT_INVALID;
    }
    // find removes from ready list
    taken_tids[tcb_to_zombify->tid] = 0;
    zombie_list.push_back(tcb_to_zombify);
  }
